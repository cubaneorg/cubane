...
from cubane.svgicons.templatetags.svgicon_tags import svgiconref
import os
import codecs


class SvgIconsTemplateTagsInlineSvgIconsTestCase(CubaneTestCase):
    """
    cubane.svgicons.templatetags.svgicon_tags.inline_svgicons()
    """
    @override_settings(DEBUG=True)
    def test_should_fail_without_target(self):
        with self.assertRaisesRegexp(AttributeError, 'Expected valid \'target\' argument for template tag'):
            inline_svgicons(None)


    @override_settings(DEBUG=True)
    def test_should_ignore_unknown_target(self):
        with self.assertRaisesRegexp(AttributeError, 'Expected valid \'target\' argument for template tag'):
            inline_svgicons('undefined-target')


    @override_settings(DEBUG=True)
    def test_should_collect_svg_resources(self):
    ...
    def test_should_serve_cached_file_content_generated_at_deployment(self):
...
class SvgIconsTemplateTagsSvgIconTestCase(CubaneTestCase):
    """
    cubane.svgicons.templatetags.svgicon_tags.svgicon()
    """
    def test_should_fail_without_name(self):
        with self.assertRaisesRegexp(AttributeError, 'Expected valid \'name\' argument for template tag \'svgicon\'.'):
            svgicon(None)


    def test_should_render_svg_use_markup_for_inlined_svg_icon_set(self):
        self.assertEqual(
            '<i class="svgicon svgicon-mail"><svg><use xlink:href="#svgicon-mail"/></svg></i>',
            svgicon('mail')
        )


class SvgIconsTemplateTagsSvgIconRefTestCase(CubaneTestCase):
    """
    cubane.svgicons.templatetags.svgicon_tags.svgiconref()
    """
    def test_should_fail_without_ref(self):
        with self.assertRaisesRegexp(AttributeError, 'Expected valid \'ref\' argument for template tag \'svgiconref\'.'):
            svgiconref(None)


    def test_should_fail_without_target_ref(self):
        with self.assertRaisesRegexp(AttributeError, 'Expected valid \'ref\' argument for template tag \'svgiconref\' in the format \'<target>/<icon-name>\'.'):
            svgiconref('email')


    @override_settings(DEBUG=True)
    def test_should_render_svg_use_markup_via_media_api_in_debug_mode(self):
        self.assertEqual(
            '<i class="svgicon svgicon-email"><svg><use xlink:href="/media-api/svgicons/frontend/email.svg#svgicon-email"/></svg></i>',
            svgiconref('frontend/email')
        )


    @override_settings(DEBUG=False)
    def test_should_render_svg_use_markup_referencing_svg_icon_file_in_production_mode(self):
        identifier = load_resource_version_identifier()
        filename = get_svgicons_filename('frontend', identifier)
        self.assertEqual(
            '<i class="svgicon svgicon-email"><svg><use xlink:href="/static/%s#svgicon-email"/></svg></i>' % filename,
            svgiconref('frontend/email')
        )
