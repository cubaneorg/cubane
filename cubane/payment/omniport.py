...
class OmniPortPaymentGateway(PaymentGateway):
    MIN_DEPOSIT_PERCENTAGE = 10
    MAX_DEPOSIT_PERCENTAGE = 50
    MINIMUM_TOTAL = Decimal('277.78')
    LIVE_DOMAIN = 'https://omniport.omnicapital.co.uk'
    TEST_DOMAIN = 'https://omniporttest.ocrf.co.uk'
    LIVE_URL = '%s/credit_app/Default.aspx' % LIVE_DOMAIN
    TEST_URL = '%s/credit_app/Default.aspx' % TEST_DOMAIN
    ...
    def register_payment(self, request, order, card_details=None, preauth=False):
        ...
        if 'installation_id' not in config:
            raise Exception('[Omni]: Missing INSTALLATION ID in settings file.')

        finance_option = order.finance_option
        if not finance_option:
            raise Exception('[Omni]: FINANCE CODE is not provided in order.')
        finance_code = finance_option.code
        if OmniPortPaymentGateway.MINIMUM_TOTAL > order.total_payment:
            raise Exception('[Omni]: TOTAL IS SMALLER THAN MINIMUM TOTAL.')
        ...
        if is_order_valid:
            post = {
                ...
                'Goods[0][Description]': 'Order #%s' % order.order_id,
                'Goods[0][Price]': order_total,
            ...
            r = requests.post(self._get_url(), post, allow_redirects=True)
            print r.url
            ...
            raise Exception('[Omni]: Order is not valid check minimums, deposit percentage.')
    ...
    def get_secret_id(self, request):
        ...
        if not secret_id:
            raise Exception('[Omni]: Secret ID doesn\'t exists.')
    ...
    def payment_update(self, request):
        ...
        if is_valid_request:
            ...
            raise Exception('[Omni]: Payment Update couldn\'t update given order.')
    ...
    def _get_api_key(self):
        ...
        if 'api_key' not in config:
            raise Exception('[Omni]: Missing API KEY in settings file.')
        else:
